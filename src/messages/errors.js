...
module.exports = {
  ...
  'no_input_files': () => ({
    ...
    'text': `Double-check the ${chalk.underline('files')} property in the ` +
            `configuration. This should either be a single glob pattern, or an ` +
            `array of patterns. Looks like there are no files matching this ` +
            `pattern. Btw, the current working dir is ${process.cwd()}`
  }),
